...
func TestApplyGroupModelsPatch(t *testing.T) {
	t.Run("sets a pool allowlist preserving other pools", func(t *testing.T) {
		...
		err := applyGroupModelsPatch(&settings, map[string]any{
			"kimi": []any{"kimi-k3"},
		})
	...
	t.Run("null removes a single pool entry", func(t *testing.T) {
		...
		err := applyGroupModelsPatch(&settings, map[string]any{"kimi": nil})
		require.NoError(t, err)
	...
	t.Run("null for the whole field clears every entry", func(t *testing.T) {
		...
		require.NoError(t, applyGroupModelsPatch(&settings, nil))
	...
	t.Run("empty list is stored as explicit empty", func(t *testing.T) {
		...
		require.NoError(t, applyGroupModelsPatch(&settings, map[string]any{
			"kimi": []any{},
		}))
	...
	t.Run("rejects non-object values", func(t *testing.T) {
		settings := settingsWithGroupModels(nil)
		assert.Error(t, applyGroupModelsPatch(&settings, "kimi"))
		assert.Error(t, applyGroupModelsPatch(&settings, []any{"kimi"}))
		assert.Error(t, applyGroupModelsPatch(&settings, 5))
	})

	t.Run("rejects non-list group values", func(t *testing.T) {
		settings := settingsWithGroupModels(nil)
		assert.Error(t, applyGroupModelsPatch(&settings, map[string]any{"kimi": "a"}))
		assert.Error(t, applyGroupModelsPatch(&settings, map[string]any{"kimi": 1}))
		assert.Error(t, applyGroupModelsPatch(&settings, map[string]any{"kimi": map[string]any{}}))
	})
}
